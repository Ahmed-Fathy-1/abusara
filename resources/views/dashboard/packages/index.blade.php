...
<main class="main-content w-full px-[var(--margin-x)] pb-8">
    ...
    <div class="grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6">
        <div class="flex justify-end space-x-2">
            ...
            <a href="{{ route('packages.create') }}" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
              <span>Create</span>
...
" class="gridjs-th gridjs-th-sort" tabindex="0"><div class="gridjs-th-content">
    description
                    </div><button tabindex="-1" aria-label="Sort column ascending" title="Sort column ascending" class="gridjs-sort gridjs-sort-neutral"></button>
                    ...
                    <tbody class="gridjs-tbody">
                        ...
                        </td><td data-column-id="Name" class="gridjs-td">{{ $package->title_en }}
                        </td>
                        <td data-column-id="Name" class="gridjs-td">{{ $package->title_ar }}
                        </td><td data-column-id="Name" class="gridjs-td">{{ $package->subtitle_en }}
                        </td><td data-column-id="Name" class="gridjs-td">{{ $package->subtitle_ar }}
                        </td><td data-column-id="Name" class="gridjs-td">{{ $package->description_en }}
                        </td><td data-column-id="Name" class="gridjs-td">{{ $package->description_ar }}
                        </td>
                        <td data-column-id="FavoriteColor" class="gridjs-td">
                            <div class="flex justify-end space-x-2">
                                <a href="{{ route('packages.edit', $package->id) }}" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
                                    <span>Edit</span>
                                </a>

                                <form action="{{ route('packages.destroy', $package->id) }}" method="post" class="inline-block" onsubmit="return confirmDelete();">
                                    @csrf
                                    @method('delete')
